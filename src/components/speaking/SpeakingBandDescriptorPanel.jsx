...
import { SPEAKING_BAND_DESCRIPTORS } from '../../content/bandDescriptors.js'

export function SpeakingBandDescriptorPanel() {
    ...
    <div className="rounded-lg border border-base-700 bg-base-850">
      ...
      {open && (
        <div className="space-y-3 border-t border-base-700 px-3 py-3">
          {Object.values(SPEAKING_BAND_DESCRIPTORS).map((crit) => (
            <div key={crit.label}>
          ...
          <p className="text-[10px] text-slate-500">Source: official IELTS Speaking Band Descriptors (public version, ielts.org).</p>
        </div>
      )}
    </div>
  )
